...
describe('TauriSource IPC contract', () => {
  ...
  it('forwards save changes and maps categories', async () => {
    ...
    // A worker payload with no `rejected` key normalises to an empty list, so the UI's
    // `rejected.length > 0` partial-save check reads a full save as fully saved.
    await expect(source.saveChanges('42', changes)).resolves.toEqual({ saved: 2, rejected: [] })
