...
- **(2026-09-25 macOS 紅綠燈與視窗圓角改原生)** 使用者要求依最新 macOS 規範修改紅綠燈與圓角。原本 macOS 也吃 `decorations: false` → tao 建成 `NSWindowStyleMask::Borderless`:直角、無系統紅綠燈,左上是 CSS 畫的 12px 假按鈕。改法:新增 `src-tauri/tauri.macos.conf.json`(Tauri 以 JSON Merge Patch 合併、**陣列整個取代**,所以整份 window 物件重寫;`src/__tests__/tauriConfig.test.ts` 鎖住除 `decorations`/`titleBarStyle`/`hiddenTitle` 外必須與 `tauri.conf.json` 一致)設 `decorations: true` + `titleBarStyle: "Overlay"` + `hiddenTitle: true`;`src-tauri/src/macos_chrome.rs` 在 setup 掛空的 unified `NSToolbar`(objc2 0.6 / objc2-app-kit 0.3,與 tao 同版本,Cargo.lock 無新 crate),讓紅綠燈位置與工具列視窗圓角都由當前 macOS 決定。**macOS 27 AppKit 實測**:按鈕 14pt、x=19/42/65、top=19 → 圓心 y=26,工具列列高 52pt;hitTest 確認整排除紅綠燈外點擊都落在 `WryWebView`,工具列不擋網頁、拖曳照舊走 `data-tauri-drag-region`。`TitleBar.tsx` mac 分支改 52px 列 + 1px 底線,Tauri 內不畫假按鈕;web demo 仍畫替身(aria-hidden、純裝飾,尺寸/間距對齊原生,hover 顯示 ×/−/全螢幕箭頭 SVG)。Windows 路徑未動。驗證:dev app 實跑截圖(原生紅綠燈、系統陰影、四角原生圓角)、web demo 量測替身座標與原生一致、`npm test` 91 pass、tsc、oxlint、src-tauri fmt/clippy/test 全綠。Playwright 這次本機沒跑成:`npm ci` 後需要 chromium build 1243,尚未 `npx playwright install`。
- **(2026-09-25 DMG 安裝視窗太醜)** 使用者回報 release DMG 開起來是 920×436 的空白 Finder 視窗、圖示擠在左上。根因:Tauri bundler 偵測到 `CI=true` 就對 `bundle_dmg.sh` 加 `--skip-jenkins`,跳過設定視窗/背景/圖示位置的 Finder AppleScript——v1.4.0 官方 DMG 裡沒有 `.DS_Store` 也沒有 `.background`。修法:`tauri.conf.json` 加 `bundle.macOS.dmg`(660×400、app (170,140)、Applications (490,140)、`dmg/background.tiff`);`release.yml` 的 macOS build step 設 `TAURI_BUNDLER_DMG_IGNORE_CI=true` + `timeout-minutes: 45`(AppleScript 等 `.DS_Store` 是無限迴圈),之後新增一步掛載 DMG 檢查 `.background/background.tiff` 存在且 `.DS_Store` 內含該檔名,沒有就讓 release 失敗。背景原始檔 `src-tauri/dmg/background.svg`,`node scripts/dmg-background.mjs` 用 Playwright Chromium 輸出 1x/2x 再 `tiffutil -cathidpicheck` 合成 Retina TIFF。**實測重點**:(1) 有背景圖時 Finder 圖示標籤一律黑字,深色模式也一樣,所以背景必須淺色;(2) `windowSize` 高度含標題列(本機約 32pt),內容區只有約 368pt,重要內容放 y≤340。驗證:本機以 `CI=true TAURI_BUNDLER_DMG_IGNORE_CI=true npx tauri build --bundles app dmg`(暫關 `createUpdaterArtifacts`)實際打包、掛載截圖確認外觀;workflow 的檢查腳本對新 DMG 通過、對 v1.4.0 官方 DMG 正確失敗。GitHub macOS runner 上 Finder AppleScript 能否跑,要等下一次 release 實跑才確認。
- **(2026-09-13 app 內過渡動畫,使用者選「B 標準」)** 動效 token 統一放 `index.css` 的 `:root`:`--m-fast 120ms`、`--m-base 160ms`、`--m-slow 260ms`、`--m-ease cubic-bezier(.2,.7,.2,1)`,`prefers-reduced-motion: reduce` 時三個時長全歸 0ms(spinner 的 `dcspin` 刻意保留字面值)。原本散在 16 個檔的 `.12s/.15s/.3s/.35s` inline transition 全改用 token(29 處替換,grep 確認無殘留)。新增 mount-only 進場動畫:`.dc-page`(`dcPageIn`,淡入 + 6px 上滑)掛在 Library 根、GameScreen 的 scroll container、Settings 根;`.dc-swap`(`dcFadeIn`,只淡入)掛在成就 grid/list 兩個分支根與 GameScreen 新增的 `TabOutlet`(以 pathname 是否 `/stats` 為 key 的包裝 div,獨立成小元件所以換分頁只有它重繪,GameScreen/GameHeader 不動;sticky toolbar 的 containing block 變成它,但它涵蓋整段分頁內容,行為不變)。ConfirmDialog overlay `dcFadeIn`(fast)+ 卡片 `dcScaleIn`(.98→1),UpdateBanner `dcSlideDown`,Toast 改用 `--m-slow`;成就卡片/列與勾選圓圈的 opacity/background/border 切換有 fast 過渡(`styles.ts` CARD_BASE、ckBase、`achievementView.ts` rowStyle)。驗證:typecheck、oxlint、vitest 19 檔 88、build、chromium e2e 16/16(port 5199 臨時 config,5173/5174 都被 mini-macau 佔走),dev server 在 Browser pane 用 getComputedStyle 逐項確認 token/動畫名稱/時長/easing、換分頁時 `.dc-swap` 確實 remount、對話框 overlay/card 動畫正確,並把動畫暫停在 60 ms 截圖看到半透明的中間態。注意:Browser pane 隱藏時 `requestAnimationFrame` 不會觸發,頁內腳本要用 setTimeout。
- **(2026-09-13 啟動白視窗)** 使用者回報啟動時白視窗一陣子。實測 debug build:視窗約 1.5 s 出現但整片白,3 s 後才畫好。原因:`tauri.conf.json` 的 window 一建立就可見且沒有底色,而 app 所有顏色都靠 React 掛載後 `AppLayout` 的 inline CSS 變數(`background: var(--win)`),index.html/index.css 沒有任何底色,所以 WebView2 啟動 + 載入 bundle + 首次 render 期間就是 WebView2 預設的白;Rust 端不是原因(`setup` 只在 debug 註冊 log plugin,command 全走 `spawn_blocking`)。修法:window 加 `visible: false` + `backgroundColor: "#0f141b"`(深色 `--win`);`lib/appWindow.ts` 新增 `winShow()`(`show()` + `setFocus()`,web 上 no-op),由 `AppLayout` 首次 commit 的 `useEffect` 呼叫——刻意不用 `requestAnimationFrame`,隱藏中的視窗可能永遠不產生 frame;`capabilities/default.json` 補 `core:window:allow-show` / `allow-set-focus`;`lib.rs` `setup` 起一條 thread,3 秒後若視窗仍隱藏就強制 `show()`,前端出錯時 app 不會隱形。新增 `lib/__tests__/appWindow.test.ts`。驗證:typecheck、oxlint、vitest 19 檔 88 tests、vite build、cargo fmt/clippy/test 全綠;`tauri build --debug --no-bundle` 後用 PowerShell 腳本(EnumWindows 輪詢)啟動並在主視窗首次可見的瞬間截圖:修前 1.5 s 出現白視窗,修後 2.4 s 出現時已是畫好的遊戲庫,沒有白色階段。**接著使用者要 app 自己的淡入**:`AppLayout` 根元素掛 `dc-root` class,`index.css` 讓 `.dc-root > *` 在 mount 時 150 ms opacity 淡入(`dcFadeIn`,`prefers-reduced-motion` 時關閉);只淡子元素、不淡根元素,根元素的 `var(--win)` 第一幀就在,淺色主題才不會透出深色的視窗 backgroundColor。系統本身的視窗開啟動畫(約 200 ms)仍會疊在前面。實測(debug build,PowerShell 每 100 ms 抓一幀算平均亮度):視窗出現後約 50 到 300 ms 內(視機器負載,等 renderer 第一幀)亮度從純底色 62 升到 166,約 150 ms 完成,與 CSS 一致;e2e 16/16、vitest 88 全過。
